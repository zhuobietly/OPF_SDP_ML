...
def features_from_df_Y(df_Y, case_name, load_dir):
    ...
    如果你要改y和x的配对估计是在这里
    """
    gr = graphread.GraphRead(
        f"/home/goatoine/Documents/Lanyue/data/raw_data/{case_name}.m",
        weight="binary",
        add_self_loops=True
    )
    features = []
    ...
    for _, row in df_Y.iterrows():
        ...
        load_file = os.path.join(load_dir, case + '.json')
        ...
        # 用该 JSON 刷新 X（仅替换 Pd/Qd 列），A_hat 不变# gr.X 也已更新
        features.append(case_features)
    
    # A_hat：同一对象引用到每一行（不拷贝）

    features_df = pd.DataFrame(features)
    features_df["gnn_A_hat"] = [gr.A_hat] * len(df_Y)
    # X：逐行从 json 刷新（不改 gr.X，返回副本）
    features_df["gnn_X"] = df_Y["profile"].apply(
        lambda p: gr.X_update(os.path.join(load_dir, p + ".json"), inplace=False)
    )
    return features_df
