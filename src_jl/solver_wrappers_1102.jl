...
# --- load Mosek log：Iterations / Primal / Dual / Relative gap ---
# return (iters::Union{Int,Missing}, pfeas::Union{Float64,Missing},
...
function parse_mosek_log_all(logtxt::AbstractString)
    # ---- 1) (Primal/Dual infeasibility, Relative gap, Iterations, Time）----
    iters = try
    ...
    # ---- 2) if there is no Summary，read from last line of ITE table ----
    if any(ismissing, (iters, pfeas, dfeas, relgap))
        last = nothing
        for ln in eachline(IOBuffer(logtxt))
            # e.g. "16  9.0e-09  3.9e-09  2.8e-13  1.00e+00   6.097266787e+00   6.097266781e+00   9.7e-10  0.01"
...
function solve(data, model, clique_merging, case_name; 
    alpha = 3.0, id_name = nothing, tokens = nothing, 
    perturbation = nothing, id_detect = -1, file_name = "default")
    ...
    adj, cadj, lookup_index, sigma, q =
        PowerModels._chordal_extension_original_alpha(pm, nw, clique_merging, alpha)
    ...
    if file_name != "default"
        ...
        # 运行并捕获日志
        println("🔷 开始求解：$(case_name) / $(network_type) / $(model) / $(clique_merging) / α=$(alpha) / id=$(id_detect) " )
        ...
        df_core = DataFrame(
            network_type    = ["original_0"], # 例：original_0, light_1, heavy_2
