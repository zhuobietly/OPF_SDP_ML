...
function solve(data, model, clique_merging)
    ...
    result = optimize_model!(pm, optimizer=optimizer_with_attributes(Mosek.Optimizer, "MSK_DPAR_OPTIMIZER_MAX_TIME" => 300.0))
...
case = "/home/goatoine/Documents/Lanyue/data/pglib_opf_case118_ieee.m"
case_name_save= "case118"
merging_options = [true]
σ = 0.5
    ...
    for merging in merging_options
        for formulation in formulations
            try
                ...
                df = DataFrame(Formulation = ["AC_SDP"], 
                            Case = case_name, 
                            Merge = ["None"], 
                ...
                result_path = "/home/goatoine/Documents/Lanyue/result/merging_clique_experim"
                mkpath(joinpath(result_path, case_name_save))
                CSV.write(joinpath(result_path, case_name_save, "results"*case_name_save*string(formulation)*string(merging)*".csv"), df, append=true, header=false)
            catch e
